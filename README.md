...
**NOTE:** **_For a new data set, the hyperparameter `--decay-k` needs to be manually adjusted according to the maximum number of training updates or epochs (--use-epoch-numbers-decay) to ensure that the probability of sampling golden words does not decay so quickly._**
